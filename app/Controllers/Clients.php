...
class Clients extends Security_Controller {
    ...
    function modal_form() {
        ...
        if ($permission_manager->can_manage_invoices()) {
    ...
    function overview($client_id) {
        ...
        $view_data['owners_dropdown'] = $this->Users_model->get_id_and_text_dropdown(array("first_name", "last_name"), array("deleted" => 0, "status" => "active", "user_type" => "staff"));
    ...
    function update_client_info($id = 0, $data_field = "") {
        ...
        if ($data_field == "labels" || $data_field == "group_ids") {
            validate_list_of_numbers($value);
            $data = array(
                $data_field => $value
            );
