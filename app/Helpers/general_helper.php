...
if (!function_exists('log_notification')) {
    ...
    function log_notification($event, $options = array(), $user_id = 0) {
        ...
        try {
            ...
            $data = ['event'   => encode_id($event, 'notification')];
            ...
            if ($statusCode >= 200 && $statusCode < 300) {
                ...
                log_message('error', 'log_notification Event: ' . $event . ' ResponseBody: ' . $response->getBody());
            }
        } catch (\CodeIgniter\HTTP\Exceptions\HTTPException $e) {
            log_message('error', 'log_notification Event: ' . $event . ' cURL Error: ' . $e->getMessage());
        } catch (\Exception $e) {
            log_message('error', 'log_notification Event: ' . $event . ' General Error: ' . $e->getMessage());
        }
    }
}
...
if (!function_exists('fix_html_tags')) {
    function fix_html_tags($html) {
        ...
        $dom = new DOMDocument();
        // Remove mb_convert_encoding() and use htmlspecialchars_decode() instead
        $dom->loadHTML(htmlspecialchars_decode($html), LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);
...
if (!function_exists('get_statement_making_data')) {
    ...
    function get_statement_making_data($options = array()) {
        ...
        if ($client_info) {
            ...
            $data['client_info'] = $ci->Clients_model->get_one($client_id);
